...
import com.moodyclues.repository.JournalRepository;
...
public class MoodyCluesApplication {
	...
	@Bean
	CommandLineRunner ormTest(JournalUserRepository userRepo, EmotionRepository emoRepo, JournalEntryRepository entRepo, JournalRepository journRepo) {
		return args -> {
...
//			Journal journal = journRepo.findJournalByUserEmail("firstTest@email.com").orElseThrow();
//			JournalEntry entry = new JournalEntry(journal);
//			
//			int moodMorning = 1;
//			int moodAfternoon = 2;
//			int moodEvening = 5;
