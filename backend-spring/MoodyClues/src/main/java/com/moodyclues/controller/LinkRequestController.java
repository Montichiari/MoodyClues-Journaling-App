...
public class LinkRequestController {
	...
	@GetMapping("/counsellor/all-link-requests/{counsellorId}")
	public ResponseEntity<?> allLinkRequestsAndroid(@PathVariable String counsellorId) {
    ...
    public ResponseEntity<?> decide(@PathVariable String requestId,
                                       @RequestBody JournalLinkRequestDto decision,
                                       HttpSession session) {
        String journalUserId = (String) session.getAttribute("id");
        linkService.requestDecision(journalUserId, requestId, decision.isApproved());
        return new ResponseEntity<>(HttpStatus.OK);
    }
    
    @PostMapping("/{requestId}/decision/{journalUserId}")
    public ResponseEntity<?> decideAndroid(@PathVariable String requestId,
                                       @RequestBody JournalLinkRequestDto decision,
                                       @PathVariable String journalUserId) {
        linkService.requestDecision(journalUserId, requestId, decision.isApproved());
        return new ResponseEntity<>(HttpStatus.OK);
    }
